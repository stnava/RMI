...
## Tutorial Goals
* Learn about *R*
in general (operations, dataframes, models)
...
```{r,boring-plots-data,echo=FALSE,results='hide',dev='jpeg'}
...
```{r,out.width='.4\\linewidth',dev='jpeg'}
...
dataframe \newline \newline
```{r,out.width='.4\\linewidth',dev='jpeg',results='show',eval=TRUE,echo=TRUE}
...
```{r,out.width='.4\\linewidth',dev='jpeg',results='hide'}
...
```{r,out.width='.4\\linewidth',dev='jpeg',eval=TRUE,results='show',echo=FALSE}
...
```{r,myviz,echo=TRUE,eval=TRUE,warning=FALSE,results='hide',fig.width=4,fig.height=4,out.width='.8\\linewidth',dev='jpeg'}
orthographic( as.array( colina ), oma=rep(2,4))
```

## Quickly Show Images w/ANTsR
```{r,myviza,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.9\\linewidth',dev='jpeg'}
...
```{r,convvec,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
imgvec<-colina[  colina > 50  ]
print( length( imgvec ) )
```
Use  oro.nifti to convert an image to a vector.
```{r,convvec2,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
```bash
antsRegistration -d 2 -m  CC[r16slice.nii.gz,r64slice.nii.gz,1,4]
...
```{r,vreg1,echo=TRUE,eval=TRUE,results='show',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
predictor<-as.factor(
...
```{r,vreg2,echo=TRUE,eval=TRUE,results='show',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
qvals<-p.adjust(pvals,method="holm") # holm recommended over BH
...
```{r}
plot( mask, list( pvali ), threshold=c(0.99,1) )
```
The regression solution p-values thresholded at 0.01 FDR-corrected.

**Should use `bigLMStats` - much faster.**


##  .... multivariate statistics in {\bf R}
\transdissolve
```{r,multivar,echo=TRUE,results='hide',warning=FALSE}
...
##  Visualizing multivariate statistics in {\bf R}
\transdissolve

```{r sccanviz,echo=FALSE,out.width='.8\\linewidth'}
plot( mask, list( sccansol ), threshold=c(0.05,1.1) )
```
The sccan solution with p-value `r sccan$ccasummary$Variate000[1]`
...
Easy solution in *R*
(w/o penalties):
```{r,out.width='.4\\linewidth',dev='jpeg',eval=FALSE,echo=TRUE,show='hide'}
mysolution=svd(X)
...
```{r,out.width='.8\\linewidth',dev='jpeg',eval=TRUE,show='hide',echo=FALSE}
...
Also easy in *R*
(SVD used internally):
```{r,out.width='.4\\linewidth',dev='jpeg',echo=TRUE,eval=TRUE,results='hide'}
...
```{r,out.width='.4\\linewidth',dev='jpeg',results='hide',eval=TRUE,echo=FALSE}
...
```{r,out.width='.4\\linewidth',dev='jpeg',eval=TRUE,results='hide',echo=TRUE}
inmask <-  mask > 0.5
...
## Prediction: PBAC---Cognition from Brain
```{r,predictpbac2,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='show',fig.width=4,fig.height=4,out.width='.7\\linewidth',dev='pdf'}
...
```{r,myhelpers1,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
from its average.
```{r,myhelpers2,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
```{r,myhelpers3,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
```{r,boldmotor1,echo=FALSE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
```{r,boldmotor2,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
```{r,boldmotor3,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
  avg<-myvars$avgImage
  mask<-myvars$mask
...
```{r,boldmotor4,echo=TRUE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
# see http://mumford.fmripower.org/lev1.pdf
  mat  <- myvars$matrixTimeSeries
# mat <- timeseries2matrix( fmri, mask )
# should also filter the hrf ?  should normalize each column to constant?
  mat  <- filterfMRIforNetworkAnalysis( cbind(mat) , 2.5, cbfnetwork = "BOLD" , freqLo=0.001 , freqHi = 0.5  )$filteredTimeSeries
...
## BOLD fMRI Motor Tasks 4
```{r,boldmotor5,echo=FALSE,warning=FALSE,eval=TRUE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
  mx<-max( betas )
 ...
 if ( ! exists("mymni") ) {
  ...
  bth=mean(betaimg[betaimg>0])+sd( betaimg[betaimg>0]  )
  clust <- labelClusters( betaimg , minClusterSize=10,
    minThresh= bth, maxThresh=100)
  plot( avg, clust,  axis = 3, window.overlay=c(1,max(clust)))
```
```{r,boldmotor6,echo=TRUE,warning=FALSE,eval=FALSE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
```{r,boldmotor8,echo=TRUE,warning=FALSE,eval=FALSE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
nvecs<-11
ff <- sparseDecom( mat[!is.na(hrf), ], mask, 1.25/nvecs, nvecs,
    its = 5, cthresh = 5, smooth = 1, z = -0.9  )
for ( i in 1:nvecs ) {
  print(paste("Test",i))
  mdl<-lm( ff$projections[,i]  ~  . , data =  data.frame( desmat ) )
  print( summary( mdl ) )
  }
...
```{r,boldmotor8b,echo=FALSE,warning=FALSE,eval=FALSE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
Use multivariate decomposition to help understand both the data and
the nuisance variables.
```{r,boldmotor9,echo=1,warning=FALSE,eval=FALSE,tidy=TRUE,results='show',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
Use multivariate decomposition to help understand both the data and
the nuisance variables.
```{r,boldmotor10,echo=1,warning=FALSE,eval=FALSE,tidy=TRUE,results='show',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
...
Use multivariate decomposition to help understand both the data and
the nuisance variables.
```{r,boldmotor9v,echo=1,warning=FALSE,eval=FALSE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
  eigimg <- ff$eigenanatomyimages[[2]]
  eigimg=iMath(eigimg,"Normalize")
plot(myantsimage = avg, functional = list(eigimg),
  axis = 3, threshold = c(0.01,1),
  col="red",outname="figure/antsrsdecom2.jpeg")
```
\begin{figure}[b]
...
Use multivariate decomposition to help understand both the data and
the nuisance variables.
```{r,boldmotor10v,echo=1,warning=FALSE,eval=FALSE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
eigimg <- ff$eigenanatomyimages[[1]] %>% iMath("Normalize")
plot( avg, list(eigimg),  axis = 3, threshold = c(0.1,1) )
```
\begin{figure}[b]
...
```{r,boldlang,echo=TRUE,warning=FALSE,eval=FALSE,tidy=TRUE,results='hide',fig.width=4,fig.height=4,out.width='.4\\linewidth',dev='jpeg'}
if ( FALSE ) {
...
print( mx )
th<-c(3,mx)
plot( avg,  list(betaimg),  axis = 3, threshold = th )
